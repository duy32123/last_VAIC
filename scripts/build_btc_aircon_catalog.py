...
def _read_shared_strings(zf: zipfile.ZipFile) -> list[str]:
    ...
    return [_xml_text(si) for si in root.findall("{*}si")]

def _sheet_path_from_workbook(zf: zipfile.ZipFile, sheet_name: str) -> str:
    workbook = ET.fromstring(zf.read("xl/workbook.xml"))
    rels = ET.fromstring(zf.read("xl/_rels/workbook.xml.rels"))
    rel_by_id = {rel.attrib["Id"]: rel.attrib["Target"] for rel in rels.findall("{*}Relationship")}
    ...
    raise SystemExit(f'Missing required sheet "{sheet_name}" in workbook')

def _coerce_xlsx_value(raw: str | None, cell_type: str | None, shared_strings: list[str]) -> Any:
    if raw is None:
        return None
    if cell_type == "s":
...
def _load_workbook_rows_stdlib(path: Path) -> tuple[list[str], Iterable[tuple[int, tuple[Any, ...]]]]:
    with zipfile.ZipFile(path) as zf:
        shared_strings = _read_shared_strings(zf)
        sheet_path = _sheet_path_from_workbook(zf, SHEET_NAME)
        root = ET.fromstring(zf.read(sheet_path))
    parsed_rows: list[tuple[int, tuple[Any, ...]]] = []
    for row in root.findall(".//{*}sheetData/{*}row"):
        row_number = int(row.attrib.get("r", len(parsed_rows) + 1))
        values: list[Any] = []
        for cell_el in row.findall("{*}c"):
            ref = cell_el.attrib.get("r", "A")
            col_index = _xlsx_col_index(ref)
            while len(values) <= col_index:
                values.append(None)
            value_el = cell_el.find("{*}v")
            inline_el = cell_el.find("{*}is")
            raw = value_el.text if value_el is not None else (_xml_text(inline_el) if inline_el is not None else None)
            values[col_index] = _coerce_xlsx_value(raw, cell_el.attrib.get("t"), shared_strings)
        parsed_rows.append((row_number, tuple(values)))
    if not parsed_rows:
        raise SystemExit(f'Sheet "{SHEET_NAME}" in {path} has no rows')
    headers = [str(x or "") for x in parsed_rows[0][1]]
    return headers, iter(parsed_rows[1:])

def load_workbook_rows(path: Path) -> tuple[list[str], Iterable[tuple[int, tuple[Any, ...]]]]:
    try:
        from openpyxl import load_workbook
    except ImportError:
        return _load_workbook_rows_stdlib(path)
